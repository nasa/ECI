 ...
 * Text: "Cmd Queue for mid 0x%04X is full. Msg dropped",
 * MsgRcv[idx].MsgStruct->mid<br> Type: Error<br> Cause: Command queue for mid
 * has filled-up during currently control cycle and cannot<br> accept another
 * command message
 */
 ...
 * Text: "Msg with Invalid MID Received on %s: ID = 0x%X", (PipeType ? "DATA
 * PIPE":"CMD PIPE"), mid<br> Type: Error<br> Cause: Message with unsubscribed
 * mid received on app SB pipe<br>
 */
#define ECI_UNK_MID_ERR_EID 154

/**
 * Description:  Error getting parameter table address from cFE table
 * services<br> Text: "Table Manage Cmd ERROR: Getting ECI Parameter Table
 * Address(idx = %d) with %s, Status = 0x%08X", <br>
 *      (int)CmdPtr->Payload.Parameter,
 * ECI_ParamTable[CmdPtr->Payload.Parameter].tblname, (int)Result Type:
 * Error<br> Cause: After table update, app received error from
 * CFE_TBL_GetAddress() for updated<br> table data.
 */
#define ECI_MANAGE_TBL_GETADDR_ERR_EID 155

/**
 * Description:  Received internal table management notification command from
 * cFE table services<br> with invalid index parameter Text: "Table Manage Cmd
 * ERROR: ECI Parameter Table idx (idx = %d) out of valid range (0 to %d)", <br>
 *      (int)CmdPtr->Payload.Parameter, (int)(SIZEOF_ARRAY(ECI_ParamTable) - 1)
 * Type: Error<br>
 * Cause: cFE table services sent an internal table management notification
 * command with an invalid<br> index parameter for the ECI_ParamTable[] array.
 */
#define ECI_MANAGE_TBL_PARAM_IDX_ERR_EID 156

/**
 * Description:  Error subscribing to the ECI_CMD_MID<br>
 * Text: "Error Subscribing to ECI_CMD_MID 0x%04X, Status = 0x%08X",
 * ECI_CMD_MID, status<br> Type: Error<br> Cause: CFE_SB_Subscribe() returns an
 * error when subscribing to ECI_CMD_MID<br>
 */
#define ECI_SUB_CMD_MID_ERR_EID 157

/**
 * Description:  Error subscribing to ECI_SEND_HK_MID<br>
 * Text: "ERROR Subscribing to ECI_SEND_HK_MID 0x%04X, Status = 0x%08X",
 * ECI_SEND_HK_MID, status<br> Type: Error<br> Cause: CFE_SB_Subscribe() returns
 * an error when subscribing to ECI_SEND_HK_MID<br>
 */
 ...
 * Type: Error<br>
 * Cause: CFE_SB_Subscribe() returns an error when subscribing to
 * ECI_TBL_MANAGE_MID<br>
 */
#define ECI_SUB_TBL_MANAGE_MSG_ERR_EID 159

/**
 * Description:  Error subscribing to ECI_TICK_MID<br>
 * Text: "Error Subscribing to ECI_TICK_MID 0x%04X, Status = 0x%08X",
 * ECI_TICK_MID, status<br> Type: Error<br> Cause: CFE_SB_Subscribe() returns an
 * error when subscribing to ECI_TICK_MID<br>
 */
#define ECI_SUB_TICK_MSG_ERR_EID 160

/**
 * Description:  MID in ECI_MsgRcv[idx] is a reserved value<br>
 * Text: "Msg ID for CMD MID 0x%04X in ECI_MsgRcv[%d] is reserved value",
 * MsgRcv[idx].MsgStruct->mid, idx<br> Type: Error<br> Cause:  MID in
 * ECI_MsgRcv[idx] equals ECI_SEND_HK_MID, ECI_TBL_MANAGE_MID, or
 * ECI_TICK_MID<br>
 */
 ...
 * Text: "Error Subscribing to CMD MID 0x%04X in ECI_MsgRcv[%d] on CMD Pipe,
 * Status = 0x%08X", <br> MsgRcv[idx].MsgStruct->mid, idx, status Type:
 * Error<br> Cause: CFE_SB_Subscribe() returns an error when subscribing to tlm
 * mid in ECI_MsgRcv[idx]<br>
 */
#define ECI_CMD_SUBSCRIBE_ERR_EID 162

/**
 * Description:  Error subscribing to telemetry message in ECI_MsgRcv[idx]<br>
 * Text: "Error Subscribing to MsgID 0x%04X in ECI_MsgRcv[%d] on Data Pipe,
 * Status = 0x%08X",<br> MsgRcv[idx].MsgStruct->mid, idx, status Type: Error<br>
 * Cause: CFE_SB_Subscribe() returns an error when subscribing to cmd mid in
 * ECI_MsgRcv[idx]<br>
 */
#define ECI_TLM_SUBSCRIBE_ERR_EID 163

/**
 * Description:  Error registering table ECI_ParamTable[idx] with cFE table
 * services<br> Text: "ERROR Registering ECI_ParamTable[%d], Status = 0x%08X",
 * idx, (int)param_status<br> Type: Error<br> Cause: CFE_TBL_Register() returns
 * an error for ECI_ParamTable[idx]<br>
 */
#define ECI_PARAM_TBL_REG_ERR_EID 164

/**
 * Description:  Error loading table ECI_ParamTable[idx] with default table file
 * data<br> Text: "ERROR Loading ECI_ParamTable[%d] (%s), Status = 0x%08X",<br>
...
/**
 * Description: Error getting address for ECI_ParamTable[idx] after table data
 * load<br> Text: "ERROR Getting Address from cFE Table Services from
 * ECI_ParamTable[%d] (%s), Status = 0x%08X",<br> idx,
 * ECI_ParamTable[idx].tblname, (int)status Type: Error<br> Cause:
 * CFE_TBL_GetAddress() returns error for ECI_ParamTable[idx] after table load
 ...
 *      idx, ECI_ParamTable[idx].tblname, (int)status
 * Type: Error<br>
 * Cause: CFE_TBL_Manage() returns error for ECI_ParamTable[idx] after table
 * load during initialization<br>
 */
#define ECI_PARAM_TBL_MANAGE_ERR_EID 167

/**
 * Description:  Application received cmd msg with an invalid command code<br>
 * Text: ECI_INV_CMD_CODE_ERR_EID, CFE_EVS_ERROR, "Invalid Command Code: ID =
 * 0x%X, CC = %d", messageID, commandCode<br> Type: Error<br> Cause: Application
 * received a cmd message with a valid messageID, but with a command code that
 * was not within<br> the valid range (ECI_FUNC_CODE_START - ECI_FUNC_CODE_END)
 */
 ...
 * Type: Error<br>
 * Cause: CFE_SB_RcvMsg() returns an error while pending forever on the command
 * pipe<br>
 */
 ...
 * Text: "ECI_CDS[%d] failed to restore from CDS, Status = 0x%08X", idx,
 * status<br> Type: Error<br> Cause: CFE_ES_RestoreFromCDS() returns an error
 * when restoring data from CDS<br>
 */
 ...
 */
/**
 * Description:  Application has a command/telemetry message buffered or
 * queued<br> Text: "Mid 0x%04X is %s", ((status == QUEUED) ?
 * "QUEUED":"BUFFERED"), MsgRcv[idx].MsgStruct->mid<br> Type: DEBUG<br> Cause:
 * Application has a command/telemetry message buffered or queued<br>
 */
