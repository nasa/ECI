...
void OP_Test_tableValidate()
{
     ...
     * not sent
     */
    UtAssert_True(Ut_CFE_EVS_GetEventCount(1, CFE_EVS_ERROR, "") == 0,
                  "Event for TLE Line1 failing validation did not increment");
    UtAssert_True(Ut_CFE_EVS_GetEventCount(2, CFE_EVS_ERROR, "") == 0,
                  "Event for TLE Line2 failing validation did not increment");
    UtPrintf("Line1 Failed: %d, computed: %d, expected: %d\n", line1Check.failed, line1Check.computed,
             line1Check.expected);
    UtPrintf("Line2 Failed: %d, computed: %d, expected: %d\n", line2Check.failed, line2Check.computed,
             line2Check.expected);
