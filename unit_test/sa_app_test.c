...
void SA_AppMain_Test_TBL_1010(void)
{
    ...
    /* Using CFE_TBL_Manage() stub which indicates table update is pending - Place Valid Table management messages on
     * command message */
...
void SA_AppMain_Test_CMD_1050_1051_1052_1053(void)
{
    ...
    UtAssert_True(HkPacket->CmdErrorCounter == 3,
                  "Command Rejected Counter Incremented Correctly for Command Messages that overflow Msg Queue");
    UtAssert_True(HkPacket->CmdAcceptCounter == 25,
                  "Command Accepted Counter Incremented Correctly for Command Messages that are Queued");
...
void SA_AppMain_Test_FDC_1001_1030(void)
{
    ...
    UtAssert_True(HkPacket->CmdErrorCounter == 2,
                  "Command Rejected Counter Incremented for two invalid FDC Config Command");
    ...
    UtAssert_True(HkPacket->Latched[0] == 1,
                  "FDC Latched State Set due to FDC Condition Met via input telemetry message");
    ...
    UtAssert_True(FdcPacket->Tlm.Data[0] == 0,
                  "FDC Tlm Set due to FDC Condition not being met via input telemetry message");

    UtAssert_True(
        Ut_CFE_EVS_GetEventCount(APP_FAULTREP_EVS_INV_DETECTOR_ID, CFE_EVS_ERROR,
                                 "Fault Reporter Reject Config Detector Cmd: Invalid fault ID 2 (Max ID = 0)") == 1,
        "Error event received due to FDC Config command with invalid FDC IDs");
    UtAssert_True(
        Ut_CFE_EVS_GetEventCount(APP_FAULTREP_EVS_INV_DETECTOR_ID, CFE_EVS_ERROR,
                                 "Fault Reporter Rejected Clear Detector Cmd:  Invalid fault ID 2 (Max ID = 0)") == 1,
    ...
    UtAssert_True(Ut_CFE_EVS_GetEventCount(APP_FAULTREP_EVS_CONFIG_CMD_ERR, CFE_EVS_ERROR,
                                           "Fault Reporter Reject Config Detector Cmd: Invalid enable value 2") == 1,
                  "Error event received due to FDC Config commands with invalid FDC Enable State");
